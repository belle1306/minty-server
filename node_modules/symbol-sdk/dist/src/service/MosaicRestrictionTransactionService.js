...
const MosaicRestrictionEntryType_1 = require("../model/restriction/MosaicRestrictionEntryType");
...
class MosaicRestrictionTransactionService {
    ...
    getAddressRestrictionEntry(mosaicId, restrictionKey, targetAddress) {
        return this.restrictionMosaicRepository.search({ mosaicId, targetAddress, entryType: MosaicRestrictionEntryType_1.MosaicRestrictionEntryType.ADDRESS }).pipe(operators_1.map((mosaicRestriction) => {
            var _a;
            const addressRestriction = mosaicRestriction.data.find((r) => r instanceof MosaicAddressRestriction_1.MosaicAddressRestriction && r.mosaicId.equals(mosaicId) && r.targetAddress.equals(targetAddress));
    ...
    getGlobalRestrictionEntry(mosaicId, restrictionKey) {
        return this.restrictionMosaicRepository.search({ mosaicId, entryType: MosaicRestrictionEntryType_1.MosaicRestrictionEntryType.GLOBAL }).pipe(operators_1.map((mosaicRestrictionPage) => {
            const globalRestriction = mosaicRestrictionPage.data.find((r) => r instanceof MosaicGlobalRestriction_1.MosaicGlobalRestriction && r.mosaicId.equals(mosaicId));
