...
class Listener {
    ...
    transactionSubscription(channel, unresolvedAddress, transactionHash, subscribeMultisig = false) {
        return this.subscribeWithMultig(unresolvedAddress, channel, subscribeMultisig).pipe(operators_1.switchMap((subscribers) => {
            return this.messageSubject.asObservable().pipe(operators_1.filter((listenerMessage) => listenerMessage.channelName === channel), operators_1.filter((listenerMessage) => listenerMessage.message instanceof Transaction_1.Transaction), operators_1.distinctUntilChanged((prev, curr) => {
                ...
                if (subscribers.includes(_.channelParam.toUpperCase())) {
                    ...
                    return transactionObservable.pipe(operators_1.filter((transaction) => transaction.isSigned(unresolvedAddress) || transaction.shouldNotifyAccount(unresolvedAddress)));
                }
            }));
        }));
    }
    ...
    transactionHashSubscription(channel, unresolvedAddress, transactionHash, subscribeMultisig = false) {
        return this.subscribeWithMultig(unresolvedAddress, channel, subscribeMultisig).pipe(operators_1.switchMap((subscribers) => {
            return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === channel), operators_1.filter((_) => typeof _.message === 'string'), operators_1.filter((_) => subscribers.includes(_.channelParam.toUpperCase())), operators_1.map((_) => _.message), operators_1.filter((_) => !transactionHash || _.toUpperCase() === transactionHash.toUpperCase()), operators_1.distinctUntilChanged());
        }));
    }
    /**
    ...
    status(unresolvedAddress, transactionHash) {
        return this.getResolvedAddress(unresolvedAddress).pipe(operators_1.mergeMap((address) => {
    ...
    cosignatureAdded(unresolvedAddress, subscribeMultisig = false) {
        return this.subscribeWithMultig(unresolvedAddress, ListenerChannelName.cosignature, subscribeMultisig).pipe(operators_1.switchMap((subscribers) => {
            return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName.toUpperCase() === ListenerChannelName.cosignature.toUpperCase()), operators_1.filter((_) => _.message instanceof CosignatureSignedTransaction_1.CosignatureSignedTransaction), operators_1.filter((_) => subscribers.includes(_.channelParam.toUpperCase())), operators_1.map((_) => _.message));
        }));
    }
    /**
     * @internal
    ...
    subscribeWithMultig(cosigner, channel, multisig = false) {
        if (!multisig) {
            ...
            return rxjs_1.of([cosigner.plain()]);
        }
        return this.getResolvedAddress(cosigner).pipe(operators_1.mergeMap((address) => {
            return this.multisigRepository.getMultisigAccountInfo(address).pipe(operators_1.map((multisigInfo) => {
